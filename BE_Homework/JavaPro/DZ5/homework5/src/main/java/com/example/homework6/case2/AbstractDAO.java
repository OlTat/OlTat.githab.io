...
import com.example.homework6.shared.Id;
...
public abstract class AbstractDAO<T> {
    ...
    public void add(T t) {
        try {
            ...
            try (Statement stat = conn.createStatement()) {
                ...
                try (ResultSet rs = stat.executeQuery("SELECT * FROM " + table)) {
                    while (rs.next()) {
                        if (rs.isLast()) {
                            id.setAccessible(true);
                            id.setInt(t, rs.getInt(1));
    ...
    public List<T> getAll(Class<T> cls, String ...values) {
...
//                        }
                        ...
                        if (values.length != 0){
                            for (String stg: values) {
                                for (int i = 1; i <= md.getColumnCount(); i++) {
                                    String columName = md.getColumnName(i);
                                    if (columName.equals(stg)){
                                        Field field = cls.getDeclaredField(columName);
                                        field.setAccessible(true);
                                        field.set(t, rs.getObject(columName));
                                    }
                                }
